...
![Azure Automation](https://img.shields.io/badge/Azure%20Automation-E3A800?style=flat&logo=data:image/svg%2bxml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCA5NiA5NiI+PHBhdGggZmlsbD0iI2ZmZmZmZiIgZD0iTTI5LjMgNzkuNUgxLjdMNDIuNyA3LjVoMjkuM0wyOS4zIDc5LjV6TTQ4LjYgNzkuNWwxMS42LTIyLjMgMjQuMSAyMi4zSDQ4LjZ6Ii8+PC9zdmc+)
...
- Never commit ServiceNow passwords, Graph client secrets, PFX files, PFX passwords, or other credentials.
- Store secrets in Azure Key Vault and retrieve them only at runtime.
- Prefer certificate-based Exchange Online app authentication over stored usernames and passwords.
- Restrict access to the Azure Automation certificate asset and its source PFX.
- Apply least-privileged Azure RBAC to the Logic App managed identity.
- Apply least-privileged Microsoft Graph and Exchange permissions to each app registration.
- Restrict the Teams review channel to authorized analysts.
- Treat subjects, senders, recipients, Message IDs, analyst notes, and ticket content as internal security data.
- Keep test and production ServiceNow endpoints separate.
- Review graph-based campaign correlation carefully because shared senders or clusters can connect multiple subject variants.
- Rotate client secrets and certificates before expiration and update Key Vault or Automation assets without committing the replacements.

---

## Error Handling

The workflow and runbooks handle:

- No campaigns returned by KQL
- Analyst skip decisions
- Empty or malformed sender values
- Existing TABL block entries
- Missing Azure Automation certificates
- Exchange Online connection failures
- Quarantine messages that cannot be resolved
- Release verification timeouts
- Partial release success
- TABL propagation delays
- Entra users that cannot be resolved by email or SAM account name
- Missing Entra department values
- Key Vault secret retrieval failures
- ServiceNow authentication or catalog authorization failures
- Oversized Teams payloads by separating the hunting query from the campaign card

Runbook output is structured JSON so the Logic App can distinguish success, partial success, pending verification, and failure without parsing console prose.
...
- Replace tenant and application IDs.
- Replace subscription, resource group, workspace, Automation account, and managed identity names.
- Replace production and test domains and email addresses.
- Replace Teams team and channel IDs.
- Replace ServiceNow hosts, catalog item IDs, usernames, and Key Vault secret names.
- Remove connection runtime URLs, connection keys, tokens, and generated authorization metadata.
- Remove certificate thumbprints and all private-key material.
- Confirm that no analyst names, user principal names, recipients, senders, or ticket contents remain.
- Search the repository for the organization's name and domain before committing.

Example local scan:

```bash
rg -n -i \
  'example\.edu|tenant-guid|subscription-guid|service-account|client-secret' \
  .
```
